...
// Version: miXSR_FC_organic_ray_marched_Global_Illumination-1
// Capability: Portable Fibonacci bio --Ray-M global illumination driver with preset-tier budgeting and shared-mask orchestration.
...
uniform int FDRMGI_PresetTier <
 ...
 ui_category = "Fine Cell - Global Illumination - Bio Ray-M Shared Mask / 02 Main Settings";
...
uniform int FDRMGI_DebugView <
    ui_type = "combo";
 ui_label = "Debug View";
 ui_category = "Fine Cell - Global Illumination - Bio Ray-M Shared Mask / 00 Debug"; ui_category_closed = true;
    ui_items = "Final\0Local Light\0Application Mask\0Depth Gate\0Ray Budget\0Computed Mask\0";
 ui_tooltip = "Shows the final output or diagnostic masks.";
> = 0;

uniform float FDRMGI_MasterIntensity < ui_type = "slider"; ui_label = "Master Intensity"; ui_category = "Fine Cell - Global Illumination - Bio Ray-M Shared Mask / 02 Main Settings"; ui_min = 0.0; ui_max = 2.3; ui_step = 0.001; ui_tooltip = "Overall multiplier for --Ray-M Global Illumination."; > = 1.475000;
uniform float FDRMGI_GIStrength < ui_type = "slider"; ui_label = "Global Illumination Strength"; ui_category = "Fine Cell - Global Illumination - Bio Ray-M Shared Mask / 02 Main Settings"; ui_min = 0.0; ui_max = 3.0; ui_step = 0.001; ui_tooltip = "How strongly ray hits add bounced light."; > = 0.712000;
uniform float FDRMGI_RayLengthPixels < ui_type = "slider"; ui_label = "Ray Length"; ui_category = "Fine Cell - Global Illumination - Bio Ray-M Shared Mask / 03 Sub Settings"; ui_category_closed = true; ui_min = 0.5; ui_max = 64.0; ui_step = 0.01; ui_tooltip = "Maximum length of each ray in screen pixels before tier scaling."; > = 3.570000;
uniform int FDRMGI_RaySteps < ui_type = "slider"; ui_label = "Ray Steps"; ui_category = "Fine Cell - Global Illumination - Bio Ray-M Shared Mask / 03 Sub Settings"; ui_category_closed = true; ui_min = 2; ui_max = 12; ui_step = 1; ui_tooltip = "Maximum compact ray steps available to the active tier. High Fidelity defaults to the full 8-step body."; > = 3;
uniform int FDRMGI_TapCount < ui_type = "slider"; ui_label = "Tap Budget"; ui_category = "Fine Cell - Global Illumination - Bio Ray-M Shared Mask / 03 Sub Settings"; ui_category_closed = true; ui_min = 4; ui_max = FDREAM_LENS55_TAP_COUNT; ui_step = 1; ui_tooltip = "Bounded Lens55 ray direction budget. High and Ultra fidelity increase tap coverage without expanding ray-step bodies."; > = 13;
uniform float FDRMGI_Thickness < ui_type = "slider"; ui_label = "Depth Thickness"; ui_category = "Fine Cell - Global Illumination - Bio Ray-M Shared Mask / 03 Sub Settings"; ui_category_closed = true; ui_min = 0.0001; ui_max = 0.1000; ui_step = 0.0001; ui_tooltip = "Depth hit tolerance for accepting ray samples."; > = 0.010000;
uniform float FDRMGI_DepthBias < ui_type = "slider"; ui_label = "Depth Bias"; ui_category = "Fine Cell - Global Illumination - Bio Ray-M Shared Mask / 03 Sub Settings"; ui_category_closed = true; ui_min = 0.0; ui_max = 0.1000; ui_step = 0.0001; ui_tooltip = "Self-intersection bias for ray hits."; > = 0.001000;
uniform float FDRMGI_DepthFalloff < ui_type = "slider"; ui_label = "Depth Falloff"; ui_category = "Fine Cell - Global Illumination - Bio Ray-M Shared Mask / 03 Sub Settings"; ui_category_closed = true; ui_min = 0.01; ui_max = 256.0; ui_step = 0.01; ui_tooltip = "Distance falloff applied along accepted ray hits."; > = 48.000000;
uniform float FDRMGI_DepthGate < ui_type = "slider"; ui_label = "Depth Gate"; ui_category = "Fine Cell - Global Illumination - Bio Ray-M Shared Mask / 03 Sub Settings"; ui_category_closed = true; ui_min = 0.0; ui_max = 1.15; ui_step = 0.001; ui_tooltip = "Suppresses ray Global Illumination when depth has weak signal or harsh discontinuities."; > = 0.803000;
uniform float FDRMGI_FibAnchor < ui_type = "slider"; ui_label = "Fibonacci Anchor"; ui_category = "Fine Cell - Global Illumination - Bio Ray-M Shared Mask / 03 Sub Settings"; ui_category_closed = true; ui_min = 987.0; ui_max = 5226.25; ui_step = 1.0; ui_tooltip = "Anchor index for bio Fibonacci ray directions."; > = 4181.000000;
uniform float FDRMGI_OrganicFlow < ui_type = "slider"; ui_label = "bio Flow"; ui_category = "Fine Cell - Global Illumination - Bio Ray-M Shared Mask / 03 Sub Settings"; ui_category_closed = true; ui_min = 0.0; ui_max = 5; ui_step = 0.001; ui_tooltip = "Phase drift for Fibonacci-derived ray direction."; > = 4.000000;
uniform float FDRMGI_ColorBleedSaturation < ui_type = "slider"; ui_label = "Bleed Saturation"; ui_category = "Fine Cell - Global Illumination - Bio Ray-M Shared Mask / 03 Sub Settings"; ui_category_closed = true; ui_min = 0.0; ui_max = 2.0; ui_step = 0.001; ui_tooltip = "Saturation of ray-hit indirect color."; > = 1.100000;
uniform float FDRMGI_MaxContribution < ui_type = "slider"; ui_label = "Max Contribution"; ui_category = "Fine Cell - Global Illumination - Bio Ray-M Shared Mask / 03 Sub Settings"; ui_category_closed = true; ui_min = 0.0; ui_max = 4.0; ui_step = 0.001; ui_tooltip = "Upper cap for bounced light contribution."; > = 1.600000;
uniform int FDRMGI_RespectFog < ui_type = "combo"; ui_label = "Respect Fog"; ui_category = "Fine Cell - Global Illumination - Bio Ray-M Shared Mask / 01 Guards (Respect Fog/Light)"; ui_category_closed = true; ui_items = "No\0Yes\0"; ui_tooltip = "Bypasses Global Illumination in fog-dominant regions."; > = 1;
uniform int FDRMGI_RespectLight < ui_type = "combo"; ui_label = "Respect Light Emission"; ui_category = "Fine Cell - Global Illumination - Bio Ray-M Shared Mask / 01 Guards (Respect Fog/Light)"; ui_category_closed = true; ui_items = "No\0Yes\0"; ui_tooltip = "Protects emissive pixels from being dimmed or recolored."; > = 1;
uniform float FDRMGI_LightProtectStrength < ui_type = "slider"; ui_label = "Light Protect Strength"; ui_category = "Fine Cell - Global Illumination - Bio Ray-M Shared Mask / 01 Guards (Respect Fog/Light)"; ui_category_closed = true; ui_min = 0.0; ui_max = 1.15; ui_step = 0.001; ui_tooltip = "How strongly protected lights return to the source color."; > = 0.900000;
uniform float FDRMGI_DebugExposure < ui_type = "slider"; ui_label = "Debug Exposure"; ui_category = "Fine Cell - Global Illumination - Bio Ray-M Shared Mask / 00 Debug"; ui_category_closed = true; ui_min = 1.0; ui_max = 32.0; ui_step = 0.1; ui_tooltip = "Boosts debug buffer visibility."; > = 1.000000;
...
float FDRMGI_LightEmissionMask(float2 uv, float3 sourceColor)
{
    ...
    return MIXSR_SHARED_SharedLightMask(
        uv,
        sourceColor,
...
float FDRMGI_LocalLightZone(float2 uv, float3 sourceColor, float centerLightMask)
{
    ...
    zone = max(zone, FDRMGI_LightEmissionMask(uv + float2(px.x, 0.0), FDRMGI_Source(uv + float2(px.x, 0.0))));
    zone = max(zone, FDRMGI_LightEmissionMask(uv - float2(px.x, 0.0), FDRMGI_Source(uv - float2(px.x, 0.0))));
    zone = max(zone, FDRMGI_LightEmissionMask(uv + float2(0.0, px.y), FDRMGI_Source(uv + float2(0.0, px.y))));
    zone = max(zone, FDRMGI_LightEmissionMask(uv - float2(0.0, px.y), FDRMGI_Source(uv - float2(0.0, px.y))));
...
float4 FDRMGI_MainPS(float4 position : SV_Position, float2 uv : TEXCOORD) : SV_Target
{
    ...
    float lightProtectMask = FDRMGI_LightEmissionMask(uv, sourceColor);
    ...
    float computedMask = saturate(applicationMask * strength * FDRMGI_MaxContribution);
    float3 giColor = saturate(sourceColor + bounce * strength * FDRMGI_MaxContribution * applicationMask);
    giColor = MIXSR_SHARED_ProtectColor(uv, giColor, sourceColor, lightProtectMask, FDRMGI_LightProtectStrength);
    float fogBypass = MIXSR_SHARED_SharedFogBypass(uv, sourceColor, FDRMGI_RespectFog);
    ...
    if (FDRMGI_DebugView == 5) return float4(FDRMGI_DebugScalar(computedMask).xxx, 1.0);
    return float4(saturate(finalColor), 1.0);
}
technique miXSR_FC_bio_raym_global_illumination < ui_label = "Fine Cell - Global Illumination - Bio Ray-M Shared Mask"; ui_tooltip = "Portable Fibonacci bio --Ray-M Global Illumination with bounded Lens55 high/ultra fidelity tiers."; >
{
    pass
    {
